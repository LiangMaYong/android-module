...
public class AMAlarm {
	 ...
	 * ACTION_ALARM
	 */
	private static final String ACTION_ALARM = "com.liangmayong.androidmoudle.alarm";
	...
	private static class AlarmReceiver extends BroadcastReceiver {
		public static final String ACTION_ALARMKIT = "com.ibwork.receiver.alarm.action";
		private static final String ORIGIONAL_INTENT = "origional_alarm_intent";
		private static final String ORIGIONAL_TIMER = "origional_alarm_timer";
		private static final String ORIGIONAL_INTERVAL = "origional_alarm_interval";
		private static final String ORIGIONAL_ID = "origional_alarm_id";
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (ACTION_ALARMKIT.equals(intent.getAction())) {
				...
				if (datae <= 0) {
					if (type == 0) {
						AMAlarm.setActivity(context, id, triggerAtMillis, origionalIntent);
					} else if (type == 1) {
						AMAlarm.setBroadcast(context, id, triggerAtMillis, origionalIntent);
					} else if (type == 2) {
						AMAlarm.setService(context, id, triggerAtMillis, origionalIntent);
					} else if (type == 10) {
						AMAlarm.setActivity(context, id, triggerAtMillis, origionalIntent);
						AMAlarm.setActivityRepeatingWakeUp(context, id, triggerAtMillis + intervalMillis,
								intervalMillis, origionalIntent);
					} else if (type == 11) {
						AMAlarm.setBroadcast(context, id, triggerAtMillis, origionalIntent);
						AMAlarm.setBroadcastRepeatingWakeUp(context, id, triggerAtMillis + intervalMillis,
								intervalMillis, origionalIntent);
					} else if (type == 12) {
						AMAlarm.setService(context, id, triggerAtMillis, origionalIntent);
						AMAlarm.setServiceRepeatingWakeUp(context, id, triggerAtMillis + intervalMillis, intervalMillis,
								origionalIntent);
					}
				} else {
					if (type == 0) {
						AMAlarm.setActivityWakeUp(context, id, triggerAtMillis, origionalIntent);
					} else if (type == 1) {
						AMAlarm.setBroadcastWakeUp(context, id, triggerAtMillis, origionalIntent);
					} else if (type == 2) {
						AMAlarm.setServiceWakeUp(context, id, triggerAtMillis, origionalIntent);
					} else if (type == 10) {
						AMAlarm.setActivityRepeatingWakeUp(context, id, triggerAtMillis, intervalMillis,
								origionalIntent);
					} else if (type == 11) {
						...
						AMAlarm.setServiceRepeatingWakeUp(context, id, triggerAtMillis, intervalMillis,
								origionalIntent);
					}
				}
			}
